...
class ReportAccountBalanceReport(models.TransientModel):
    ...
    def clear_data(self, tb_data):
        for element in tb_data["trial_balance"]:
            ...
            if account.user_type_id.internal_group in ("asset", "equity", "expense"):
                element["balance"] = element["debit"] - element["credit"]
                element["ending_balance"] = element["debit"] - element["credit"]
            elif account.user_type_id.internal_group in ("liability", "income"):
                element["balance"] = element["credit"] - element["debit"]
                element["ending_balance"] = element["credit"] - element["debit"]

        if not tb_data["docs"].show_partner_details:
            for element in tb_data["accounts_data"]:
                account = self.env["account.account"].search(
                    [("code", "=", tb_data["accounts_data"][element]["code"])]
                )
                if account.user_type_id.internal_group in (
                    "asset",
                    "equity",
                    "expense",
                ):
                    tb_data["accounts_data"][element]["balance"] = (
                        tb_data["accounts_data"][element]["debit"]
                        - tb_data["accounts_data"][element]["credit"]
                    )
                    tb_data["accounts_data"][element]["ending_balance"] = (
                        tb_data["accounts_data"][element]["debit"]
                        - tb_data["accounts_data"][element]["credit"]
                    )
                elif account.user_type_id.internal_group in ("liability", "income"):
    ...
    def _get_account_partners(self, account_data, partners):
        ...
        if isinstance(partners, dict):
            account_partners = [p_id for p_id in partners if p_id in account_data]
